...
└── services/                      # 业务服务层 — 每个服务独立封装
    └── <service-name>/            # 服务目录（标准文件 + 可选文件）
        ├── client.ts              # 创建 ApiRequest 实例，配置网关、拦截器
...
网关地址统一在 `src/config/index.ts` 的 `GATEWAY_CONFIG` 中管理，按环境和网关两个维度组织。

**规范：**
- 新增网关地址必须添加到 `GATEWAY_CONFIG` 的所有环境中
- 服务的 `client.ts` 通过 `GATEWAY_CONFIG` 引用网关地址
- 不允许在服务代码中硬编码网关 URL
- 新建服务时，先查看 `src/config/index.ts` 确认已有哪些网关可用；如需新网关，需在所有环境中补充
...
**职责：** 创建 `ApiRequest` 实例，配置网关地址和拦截器组合。
 ...
 */
const version = 'v1'
const baseUrl = `${GATEWAY_CONFIG.<env>.<gateway>}/<url-pattern>/${version}/<service-name>`
...
| 4 | **禁止硬编码网关地址** | 必须通过 `GATEWAY_CONFIG` 引用 |
...
- [ ] `client.ts` 中正确配置了网关地址和拦截器
- [ ] `client.ts` 中确认 `successCodes` 设置（默认 200000，若不同需显式指定）
- [ ] `client.ts` 注释说明了所属网关和 baseUrl 拼接规则
...
- [ ] 如需新网关地址，已添加到 `GATEWAY_CONFIG` 的所有环境中
