...
| 特征 | 归属 | 说明 |
|------|------|------|
| 纯计算/转换/校验/格式化，无 Vue API 依赖 | ✅ `utils/` | 无状态的纯工具函数 |
| 内部使用了 `ref`/`computed`/`watch` 等 Vue 响应式 API | ❌ → `hooks/` | 应使用 `create-hook` 流程 |
| 状态需要跨页面/组件全局共享 | ❌ → `store/` | 应使用 `create-store` 流程 |
| HTTP 请求相关 | ❌ → `http/` | 应使用 `create-api` 流程 |
| 仅当前页面使用的辅助方法 | ❌ → 页面文件内 | 不需要提取到 utils |

如判断不属于 utils，**告知用户并引导到正确的方向**，不继续执行后续步骤。
